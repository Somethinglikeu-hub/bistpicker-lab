...
const LIVE_PRICE_FEED_FALLBACK = '../live_tickers_v2.json';
const MANIFEST_URL = V2_BASE + '/manifest.json';
const MANIFEST_FALLBACK = '../manifest_v2.json';
const MOBILE_SNAPSHOT_URL = V2_BASE + '/mobile_snapshot.db.gz';
const MOBILE_SNAPSHOT_FALLBACK = '../mobile_snapshot.db.gz';
...
async function downloadSnapshot(manifest, onProgress) {
  // Use manifest's own snapshot.url if present, else V2_BASE
  const baseUrl = manifest.snapshot.url ? manifest.snapshot.url.split('/').slice(0,-1).join('/') : V2_BASE;
  const fname = manifest.snapshot.filename;
  let downloadUrl = `${V2_BASE}/${fname}?t=${Date.now()}`;
  if (manifest.snapshot.url) downloadUrl = `${manifest.snapshot.url}?t=${Date.now()}`;
  let arrayBuffer;
  try {
    arrayBuffer = await fetchWithProgress(downloadUrl, onProgress);
  } catch (e) {
    console.warn('[DB] Primary snapshot download failed, trying fallback:', e.message);
    const fbUrl = `../${fname}?t=${Date.now()}`;
    arrayBuffer = await fetchWithProgress(fbUrl, onProgress);
  }
...
async function refreshLivePrices() {
  ...
  try {
    let response = null;
    try {
      response = await fetch(`${LIVE_PRICE_FEED_URL}?t=${Date.now()}`, {
        cache: 'no-store',
        signal: controller.signal
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
    } catch (e1) {
      console.warn('[Price] Primary feed failed:', e1.message, 'trying fallback');
      response = await fetch(`${LIVE_PRICE_FEED_FALLBACK}?t=${Date.now()}`, {
        cache: 'no-store',
        signal: controller.signal
      });
      if (!response.ok) throw new Error(`HTTP ${response.status} fallback`);
    }
      cache: 'no-store',
      signal: controller.signal
    });
...
async function initApp() {
  ...
  try {
    ...
    let manifest = null;
    try {
      let res = await fetch(MANIFEST_URL + '?t=' + Date.now(), { cache: 'no-store' });
      if (!res.ok) {
        console.warn('[DB] Primary manifest failed HTTP', res.status, 'trying fallback');
        res = await fetch(MANIFEST_FALLBACK + '?t=' + Date.now(), { cache: 'no-store' });
      }
      if (res.ok) {
      ...
      try {
        const r2 = await fetch(MANIFEST_FALLBACK + '?t=' + Date.now(), { cache: 'no-store' });
        if (r2.ok) {
          manifest = validateSnapshotManifest(await r2.json());
          window.feedManifest = manifest;
          console.log('[DB] Fallback manifest found:', manifest.snapshot_version);
        }
      } catch(e2) {
        console.warn('[DB] Fallback manifest also failed:', e2.message);
      }
    }
